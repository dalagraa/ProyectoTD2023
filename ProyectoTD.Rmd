...
Dig1$MHR[outliers1] <- NA
Dig1$MHR <- na.locf(Dig1$MHR)
Dig2$MHR[outliers2] <- NA
Dig2$MHR <- na.locf(Dig2$MHR)
Dig5$MHR[outliers5] <- NA
Dig5$MHR <- na.fill(Dig5$MHR, fill = "extend")
Dig6$MHR[outliers6] <- NA
Dig6$MHR <- na.locf(Dig6$MHR)
...
Dig1_filtrado <- Dig1[, !apply(Dig1, 2, function(x) all(x == 0))]
Dig2_filtrado <- Dig2[, !apply(Dig2, 2, function(x) all(x == 0))]
Dig3_filtrado <- Dig3[, !apply(Dig3, 2, function(x) all(x == 0))]
Dig4_filtrado <- Dig4[, !apply(Dig4, 2, function(x) all(x == 0))]
Dig5_filtrado <- Dig5[, !apply(Dig5, 2, function(x) all(x == 0))]
Dig6_filtrado <- Dig6[, !apply(Dig6, 2, function(x) all(x == 0))]
