...
crear_df_digital <- function(x){
  HR1 <- x[seq(from = 1, to = length(x), by = 9)][1:1300]
  HR2 <- x[seq(from = 2, to = length(x), by = 9)][1:1300]
  MHR <- x[seq(from = 3, to = length(x), by = 9)][1:1300]
  MSpo2 <- x[seq(from = 5, to = length(x), by = 9)][1:1300]
  Pdiastolica <- x[seq(from = 8, to = length(x), by = 9)][1:1300]
  Pmedia <- x[seq(from = 9, to = length(x), by = 9)][1:1300]
  Psistolica <- x[seq(from = 7, to = length(x), by = 9)][1:1300]
  TOCO <- x[seq(from = 4, to = length(x), by = 9)][1:1300]
  VCP <- x[seq(from = 6, to = length(x), by = 9)][1:1300]
  
  time <- seq(0,1299,1)/4
  df <- data.frame(HR1,HR2,MHR,MSpo2,Pdiastolica,Pmedia,Psistolica,TOCO,VCP,time)
  df <- gather(df, key = "variable", value = "value", -time)
  return(df)
}
...
```{r}
ggplot(Dig1, aes(x = time, y = value)) + 
  geom_line()+ 
  facet_wrap(~ variable, scales = "free_y") 
``` 

```{r}
valores_faltantes <- c(sum(is.na(An1)), sum(is.na(Dig1)))


hist(valores_faltantes, breaks = 2, xlab = "Ficheros",
